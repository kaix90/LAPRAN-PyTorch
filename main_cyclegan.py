...
parser.add_argument('--model', help='basic | woresnet | woresnetup', default='basic_cyclegan')
...
parser.add_argument('--w-loss', type=float, default=0.1, metavar='N.',
...
def train(epochs, trainloader, valloader):
    ...
    g4_output_measure = torch.FloatTensor(opt.batch_size, sz_input[1], m1, m2)
    ...
    if opt.cuda:
        ...
        g1_target, g2_target, g3_target, g4_target, g4_output_measure = g1_target.cuda(), g2_target.cuda(), \
                                                                        g3_target.cuda(), g4_target.cuda(),\
                                                                        g4_output_measure.cuda()
        label = label.cuda()
    ...
    for epoch in range(epochs):
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            for i in range(opt.batch_size):
                for j in range(channels):
                    g4_output_measure[i, j, :, :] = torch.from_numpy(
                np.reshape(sensing_matrix_left.dot(g4_output.data.cpu().numpy()[i,j].flatten()), (m1, m2)))

            g4_output_measure_var = Variable(g4_output_measure)
            errD_g4_fake_measure_mse = criterion_mse(g4_output_measure_var, g1_target_var)
            errD_g4_fake_bce = criterion_bce(d4_output, d4_label_var)
            errD_g4_fake_mse = criterion_mse(g4_output, g4_target_var)
            errD_g4 = opt.w_loss * errD_g4_fake_bce + (1 - opt.w_loss) / 2 * errD_g4_fake_mse + \
                      (1 - opt.w_loss) / 2 * errD_g4_fake_measure_mse
            errD_g4.backward()
        ...
        for iter_model in range(5):
            for idx, (data, _) in enumerate(trainloader, 0):
                ...
                for i in range(opt.batch_size):
                    for j in range(channels):
                        g4_output_measure[i, j, :, :] = torch.from_numpy(
                            np.reshape(sensing_matrix_left.dot(g5_output.data.cpu().numpy()[i, j].flatten()), (m1, m2)))

                g5_output_measure_var = Variable(g4_output_measure)
                errD_g5_fake_measure_mse = criterion_mse(g5_output_measure_var, g1_target_var)
                errD_g5_fake_bce = criterion_bce(d5_output, d5_label_var)
                errD_g5_fake_mse = criterion_mse(g5_output, g5_target_var)
                errD_g5 = opt.w_loss * errD_g5_fake_bce + (1 - opt.w_loss) / 2 * errD_g5_fake_mse + \
                          (1 - opt.w_loss) / 2 * errD_g5_fake_measure_mse
                errD_g5.backward()
