...
parser.add_argument('--model', help='basic | adaptiveCS | adaptiveCS_resnet', default='adaptiveCS_resnet_separate_level0')
...
if opt.model == 'adaptiveCS_resnet_separate_level0':
    import models.lapgan_adaptiveCS_resnet_level0 as lapgan
elif opt.model == 'adaptiveCS':
...
def train(epochs, trainloader, valloader):
    ...
    g0_input = torch.FloatTensor(opt.batch_size, channels, m1)
    ...
    g0_target = torch.FloatTensor(opt.batch_size, channels, img_size1, img_size1)
    ...
    # Instantiate models
    lapnet0_gen = lapgan.LAPGAN_Generator_level0(channels, opt.ngpu)
    lapnet0_disc = lapgan.LAPGAN_Discriminator_level1(channels, opt.ngpu)
    lapnet1_gen = lapgan.LAPGAN_Generator_level1(channels, opt.ngpu, channels * m1)
    ...
    weights_init(lapnet0_gen, init_type='normal'), weights_init(lapnet0_disc, init_type='normal')
    ...
    optimizer_lapnet0_gen = optim.Adam(lapnet0_gen.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    optimizer_lapnet0_disc = optim.Adam(lapnet0_disc.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    ...
    if opt.cuda:
        lapnet0_gen.cuda(), lapnet0_disc.cuda()
        ...
        g0_input, g1_input, g2_input, g3_input, g4_input = g0_input.cuda(), g1_input.cuda(), g2_input.cuda(),\
                                                           g3_input.cuda(), g4_input.cuda()
        g0_target, g1_target, g2_target, g3_target, g4_target = g0_target.cuda(), g1_target.cuda(), g2_target.cuda(),\
    ...
    for epoch in range(epochs):
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            for i in range(opt.batch_size):
                ...
                g0_target_temp = g4_target_temp[:, ::8, ::8]  # 3x8x8
                g0_target[i] = torch.from_numpy(g0_target_temp)

                for j in range(channels):
                    g0_input[i, j, :] = torch.from_numpy(sensing_matrix1[j, :, :].dot(data_array[i, j].flatten()))

            g0_input_var = Variable(g0_input)
            g0_target_var = Variable(g0_target)

            # Train disc1 with true images
            lapnet0_disc.zero_grad()
            d0_output = lapnet0_disc(g0_target_var)
            d0_label_var = Variable(label.fill_(real_label))
            errD_d0_real_bce = criterion_bce(d0_output, d0_label_var)
            errD_d0_real_bce.backward()
            d0_real_mean = d0_output.data.mean()

            # Train disc1 with fake images
            g0_output = lapnet0_gen(g0_input_var)
            d0_output = lapnet0_disc(g0_output.detach())
            d0_label_var = Variable(label.fill_(fake_label))
            errD_d0_fake_bce = criterion_bce(d0_output, d0_label_var)
            errD_d0_fake_bce.backward()
            optimizer_lapnet0_disc.step()

            # Train gen1 with fake images
            lapnet0_gen.zero_grad()
            d0_label_var = Variable(label.fill_(real_label))
            d0_output = lapnet0_disc(g0_output)
            errD_g0_fake_bce = criterion_bce(d0_output, d0_label_var)
            errD_g0_fake_mse = criterion_mse(g0_output, g0_target_var)
            errD_g0 = opt.w_loss * errD_g0_fake_bce + (1 - opt.w_loss) * errD_g0_fake_mse
            errD_g0.backward()
            optimizer_lapnet0_gen.step()
            d0_fake_mean = d0_output.data.mean()

            if idx % opt.log_interval == 0:
                print('Level %d [%d/%d][%d/%d] errD_real: %.4f, errD_fake: %.4f, errG_bce: %.4f errG_mse: %.4f,'
                      'D(x): %.4f, D(G(z)): %.4f' % (
                          0, epoch, epochs, idx, len(trainloader),
                          errD_d0_real_bce.data[0],
                          errD_d0_fake_bce.data[0],
                          errD_g0_fake_bce.data[0],
                          errD_g0_fake_mse.data[0],
                          d0_real_mean,
                          d0_fake_mean))
        #val(epoch, 0, channels, valloader, sensing_matrix1, sensing_matrix2, sensing_matrix3,
        #    sensing_matrix4, g0_target, g0_input, lapnet1_gen, lapnet2_gen, lapnet3_gen, lapnet4_gen,
        #    criterion_mse, y2, y3, y4)

        torch.save(lapnet0_gen.state_dict(),
                   '%s/%s/%s/model/lapnet0_gen_epoch_%d.pth' % (opt.outf, opt.dataset, opt.model, epoch))
        torch.save(lapnet0_disc.state_dict(),
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            for i in range(opt.batch_size):
                g4_target_temp = data_array[i]  # 3x64x64
                g0_target_temp = g4_target_temp[:, ::8, ::8]  # 3x8x8
                g1_target[i] = torch.from_numpy(g0_target_temp)
            ...
            # Train disc1 with fake images
            g1_input = lapnet0_gen(g0_input_var)
            g1_output = lapnet1_gen(g1_input, g0_input_var)
