...
import models.lapgan_11182017 as lapgan
...
def val(epoch, level, m1, m2, channels, valloader, sensing_matrix_left, target, g1_input, lapnet1_gen, lapnet2_gen,
    ...
    print('Test: [%d] average errG_mse: %.4f,' % (epoch, errD_fake_mse.data[0]))
...
def train(epochs, trainloader, valloader):
    ...
    lapnet_gen = lapgan.LAPGAN(channels, opt.ngpu, lapnet1_gen, lapnet2_gen, lapnet3_gen, lapnet4_gen)
    optimizer_lapnet_gen = optim.Adam(lapnet_gen.parameters(), lr=opt.lr, betas=(0.5, 0.999))

    # Weight initialization
    weights_init(lapnet1_gen, init_type='xavier'), weights_init(lapnet1_disc, init_type='xavier')
    weights_init(lapnet2_gen, init_type='xavier'), weights_init(lapnet2_disc, init_type='xavier')
    weights_init(lapnet3_gen, init_type='xavier'), weights_init(lapnet3_disc, init_type='xavier')
    weights_init(lapnet4_gen, init_type='xavier'), weights_init(lapnet4_disc, init_type='xavier')
    ...
    for epoch in range(epochs):
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            errD_g1_fake_mse = criterion_mse(g1_output.detach(), g1_target_var)
            errD_d1_fake_bce = criterion_bce(d1_output, d1_label_var)
            errD_d1_fake = opt.w_loss * errD_d1_fake_bce + (1 - opt.w_loss) * errD_g1_fake_mse
            ...
            errD_g1_fake = opt.w_loss * errD_g1_fake_bce + (1 - opt.w_loss) * errD_g1_fake_mse
            errD_g1_fake.backward()
            optimizer_lapnet1_gen.step()
            d1_fake_mean = d1_output.data.mean()

            if idx % opt.log_interval == 0:
                print('Level %d [%d/%d][%d/%d] D_real_bce: %.4f, D_fake_bce: %.4f, D_fake: %.4f, '
                      'G_fake_bce: %.4f G_fake_mse: %.4f, G_fake: %.4f, D(x): %.4f, D(G(z)): %.4f' % (
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            errD_g2_fake_mse = criterion_mse(g2_output.detach(), g2_target_var)
            errD_d2_fake_bce = criterion_bce(d2_output, d2_label_var)
            errD_d2_fake = opt.w_loss * errD_d2_fake_bce + (1 - opt.w_loss) * errD_g2_fake_mse
            errD_d2_fake.backward()
            ...
            errD_g2_fake = opt.w_loss * errD_g2_fake_bce + (1 - opt.w_loss) * errD_g2_fake_mse
            errD_g2_fake.backward()
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            errD_g3_fake_mse = criterion_mse(g3_output.detach(), g3_target_var)
            errD_d3_fake_bce = criterion_bce(d3_output, d3_label_var)
            errD_d3_fake = opt.w_loss * errD_d3_fake_bce + (1 - opt.w_loss) * errD_g3_fake_mse
            errD_d3_fake.backward()
            ...
            errD_g3_fake = opt.w_loss * errD_g3_fake_bce + (1 - opt.w_loss) * errD_g3_fake_mse
            errD_g3_fake.backward()
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            errD_g4_fake_mse = criterion_mse(g4_output.detach(), g4_target_var)
            errD_d4_fake_bce = criterion_bce(d4_output, d4_label_var)
            errD_d4_fake = opt.w_loss * errD_d4_fake_bce + (1 - opt.w_loss) * errD_g4_fake_mse
            errD_d4_fake.backward()
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            g1_input_var = Variable(g1_input)
            g5_target_var = Variable(g4_target)
            lapnet4_disc.zero_grad()
            d5_output = lapnet4_disc(g5_target_var)
            d5_label_var = Variable(label.fill_(real_label))
            errD_d5_real_bce = criterion_bce(d5_output, d5_label_var)
            errD_d5_real_bce.backward()
            d5_real_mean = d5_output.data.mean()

            # Train lapnet_disc with fake images
            g5_output = lapnet_gen(g1_input_var, g1_input_var)
            d5_output = lapnet4_disc(g5_output.detach())
            d5_label_var = Variable(label.fill_(fake_label))
            errD_g5_fake_mse = criterion_mse(g5_output.detach(), g5_target_var)
            errD_d5_fake_bce = criterion_bce(d5_output, d5_label_var)
            errD_d5_fake = opt.w_loss * errD_d5_fake_bce + (1 - opt.w_loss) * errD_g5_fake_mse
            errD_d5_fake.backward()
            optimizer_lapnet4_disc.step()

            # Train lapnet_gen with fake images, lapgen_disc is not updated
            lapnet_gen.zero_grad()
            d5_label_var = Variable(label.fill_(real_label))
            d5_output = lapnet4_disc(g5_output)
            errD_g5_fake_bce = criterion_bce(d5_output, d5_label_var)
            errD_g5_fake_mse = criterion_mse(g5_output, g5_target_var)
            errD_g5 = opt.w_loss * errD_g5_fake_bce + (1 - opt.w_loss) * errD_g5_fake_mse
            errD_g5.backward()
            optimizer_lapnet_gen.step()
            d5_fake_mean = d5_output.data.mean()

            if idx % opt.log_interval == 0:
                print('Level %d [%d/%d][%d/%d] D_real_bce: %.4f, D_fake_bce: %.4f, D_fake: %.4f, '
                      'G_bce: %.4f G_mse: %.4f, D(x): %.4f, D(G(z)): %.4f' % (
                          5, epoch, epochs, idx, len(trainloader),
                          errD_d5_real_bce.data[0],
                          errD_d5_fake_bce.data[0],
                          errD_d5_fake.data[0],
                          errD_g5_fake_bce.data[0],
                          errD_g5_fake_mse.data[0],
                          d5_real_mean,
                          d5_fake_mean))

        val(epoch, 5, m1, m2, channels, valloader, sensing_matrix_left, g4_target, g1_input, lapnet1_gen,
            lapnet2_gen, lapnet3_gen, lapnet4_gen, lapnet_gen, criterion_mse)
