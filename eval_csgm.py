...
def evaluation(testloader):
    ...
    for idx, (data, _) in enumerate(testloader, 0):
        ...
        output_npy = output.cpu().data.numpy().squeeze() * 0.5 + 0.5
        ...
        #vutils.save_image(target_var.data, '%s/%s/cr%s/%s/test/orig_%d.bmp'
        #                  % (opt.outf, opt.dataset, opt.cr, opt.model, idx), normalize=True)
        #vutils.save_image(output.data, '%s/%s/cr%s/%s/test/recon_%d.bmp'
        #                  % (opt.outf, opt.dataset, opt.cr, opt.model, idx), normalize=True)
