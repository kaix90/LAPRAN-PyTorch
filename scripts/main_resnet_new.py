...
import argparse
import cv2
import models.lapgan_mnist_new as lapgan
...
parser.add_argument('--model', help='basic | woresnet | woresnetup', default='resnet')
...
def train(epochs, trainloader):
    ...
    lapnet1_disc.apply(weights_init)
    lapnet1_gen.apply(weights_init)
    ...
    for epoch in range(epochs):
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            optimizer_lapnet1_disc.zero_grad()
            ...
            optimizer_lapnet1_gen.zero_grad()
            d1_label_var = Variable(label.fill_(real_label))
        ...
        torch.save(lapnet1_gen.state_dict(),
                   '%s/%s/%s/lapnet_gen_epoch_%d.pth' % (opt.outf, opt.dataset, opt.model, epoch))
        torch.save(lapnet1_disc.state_dict(),
                   '%s/%s/%s/lapnet_disc_epoch_%d.pth' % (opt.outf, opt.dataset, opt.model, epoch))
        vutils.save_image(g1_target,
                          '%s/%s/%s/l%d_real_samples_epoch_%03d.png' % (opt.outf, opt.dataset, opt.model, 1, epoch),
                          normalize=True)
        vutils.save_image(g1_output.data,
                          '%s/%s/%s/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 1, epoch),
                          normalize=True)
        ...
        vutils.save_image(g2_target,
                          '%s/%s/%s/l%d_real_samples_epoch_%03d.png' % (opt.outf, opt.dataset, opt.model, 2, epoch),
                          normalize=True)
        vutils.save_image(g2_output.data,
                          '%s/%s/%s/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 2, epoch),
                          normalize=True)
        ...
        vutils.save_image(g3_target,
                          '%s/%s/%s/l%d_real_samples_epoch_%03d.png' % (opt.outf, opt.dataset, opt.model, 3, epoch),
                          normalize=True)
        vutils.save_image(g3_output.data,
                          '%s/%s/%s/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 3, epoch),
                          normalize=True)
        # training level 4
        for idx, (data, _) in enumerate(trainloader, 0):
            if data.size(0) != opt.batch_size:
        ...
        vutils.save_image(g4_target,
                          '%s/%s/%s/l%d_real_samples_epoch_%03d.png' % (opt.outf, opt.dataset, opt.model, 4, epoch),
                          normalize=True)
        vutils.save_image(g4_output.data,
                          '%s/%s/%s/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 4, epoch),
                          normalize=True)
        ...
        torch.save(lapnet4_disc.state_dict(),
                   '%s/%s/%s/lapnet_disc_epoch_%d.pth' % (opt.outf, opt.dataset, opt.model, epoch))
        vutils.save_image(g4_target,
                          '%s/%s/%s/l%d_real_samples_epoch_%03d.png' % (opt.outf, opt.dataset, opt.model, 5, epoch),
                          normalize=True)
        vutils.save_image(g4_output.data,
                          '%s/%s/%s/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 5, epoch),
                          normalize=True)
