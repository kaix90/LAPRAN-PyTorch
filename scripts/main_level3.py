...
parser.add_argument('--model', help='basic | woresnet | woresnetup', default='model_level3')
...
def val(epoch, level, m1, m2, cr, channels, valloader, sensing_matrix_left, lapnet1_gen, lapnet2_gen, lapnet3_gen,
    ...
    for idx, (data, _) in enumerate(valloader, 0):
        ...
        for i in range(opt.batch_size):
            ...
            g3_target[i] = torch.from_numpy(g3_target_temp)     # 1x8x8
        ...
        target_var = Variable(g3_target, volatile=True)
...
def train(epochs, trainloader, valloader):
    ...
    if opt.gpu != None:
        ...
        #lapnet2_param_path = '%s/%s/%s/model/lapnet2_gen_epoch_%d.pth' % (opt.outf, opt.dataset, 'model_level2', 99)
        #lapnet2_gen.load_state_dict(torch.load(lapnet2_param_path))
    ...
    for epoch in range(epochs):
        ...
        val(epoch, 3, m1, m2, cr, channels, valloader, sensing_matrix_left, lapnet1_gen, lapnet2_gen, lapnet3_gen,
            criterion_mse, opt)
...
'''
info = {
    'mse_loss': errD_g3_fake_mse.data[0],
    'D(x)': d3_real_mean,
    'D(G(z))': d3_fake_mean
}
...
for tag, value in lapnet3_gen.named_parameters():
    tag = tag.replace('.', '/')
