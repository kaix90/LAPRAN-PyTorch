...
parser.add_argument('--model', help='basic | woresnet | woresnetup', default='model1')
...
def val(epoch, level, m1, m2, channels, valloader, sensing_matrix_left, target, g1_input, lapnet1, lapnet2_gen,
    ...
    for idx, (data, _) in enumerate(valloader, 0):
        ...
        for i in range(opt.batch_size):
            ...
            if level == 2:
                target[i] = torch.from_numpy(g2_target_temp)     # 1x8x8
            elif level == 5:
                target[i] = torch.from_numpy(g4_target_temp)     # 1x32x32
