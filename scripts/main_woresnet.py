...
parser.add_argument('--model', help='basic | woresnet | woresnetup', default='woresnet')
...
def data_loader():
    ...
    if opt.dataset == 'lsun':
        ...
        test_dataset = datasets.MNIST(opt.datapath+'val', train=False, transform=transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
        ]))
...
#    torch.cuda.set_device(gpus[0])
    ...
    for epoch in range(epochs):
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            for i in range(opt.batch_size):
                g2_input[i] = torch.from_numpy(np.expand_dims(cv2.pyrUp(np.transpose(g1_output_cpu[i], (1, 2, 0))), axis=0)) # 1x16x16
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            errD_g3 = 0.2 * errD_g3_fake_bce + 0.8 * errD_g3_fake_mse
            errD_g3.backward()
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            g3_input_var = Variable(g3_input)
            g3_output = lapnet2_gen(g3_input_var)  # 1x16x16
            g3_output_cpu = g3_output.cpu().data.numpy()
            for i in range(opt.batch_size):
                g4_input[i] = torch.from_numpy(
                    np.expand_dims(cv2.pyrUp(np.transpose(g3_output_cpu[i], (1, 2, 0))), axis=0))  # 1x64x64
            # Train disc4 with true images
            g4_input_var = Variable(g4_input)
            ...
            g4_output = lapnet4_gen(g4_input_var)
            ...
            errD_g4 = 0.2 * errD_g4_fake_bce + 0.8 * errD_g4_fake_mse
            errD_g4.backward()
        ...
        torch.save(lapnet1.state_dict(), '%s/%s/g1_epoch_%d.pth' % (opt.outf, opt.model, epoch))
        torch.save(lapnet2_disc.state_dict(), '%s/%s/d2_epoch_%d.pth' % (opt.outf, opt.model, epoch))
        torch.save(lapnet2_gen.state_dict(), '%s/%s/g2_epoch_%d.pth' % (opt.outf, opt.model, epoch))
        torch.save(lapnet3_disc.state_dict(), '%s/%s/d3_epoch_%d.pth' % (opt.outf, opt.model, epoch))
        torch.save(lapnet3_gen.state_dict(), '%s/%s/g3_epoch_%d.pth' % (opt.outf, opt.model, epoch))

        vutils.save_image(g3_input, '%s/%s/real_samples_epoch_%03d.png' % (opt.outf, opt.model, epoch), normalize=True)
        vutils.save_image(g3_output.data, '%s/%s/fake_samples_epoch_%03d.png' %(opt.outf, opt.model, epoch), normalize=True)
