...
parser.add_argument('--model', help='basic | woresnet | woresnetup', default='woresnetup')
...
def train(epochs, trainloader):
    ...
    for epoch in range(epochs):
        ...
        torch.save(lapnet1.state_dict(), '%s/%s/%s/g%d_epoch_%d.pth' % (opt.outf, opt.dataset, opt.model, 1, epoch))
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            print('Level %d [%d/%d][%d/%d] errD_real: %.4f, errD_fake: %.4f, errG_bce: %.4f errG_mse: %.4f, \
                ...
                d2_real_mean,
                d2_fake_mean))
        torch.save(lapnet2_disc.state_dict(), '%s/%s/%s/d%d_epoch_%d.pth' % (opt.outf, opt.dataset, opt.model, 2, epoch))
        torch.save(lapnet2_gen.state_dict(), '%s/%s/%s/g%d_epoch_%d.pth' % (opt.outf, opt.dataset, opt.model, 2, epoch))
        vutils.save_image(g2_target, '%s/%s/%s/l%d_real_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 2, epoch),
                          normalize=True)
        vutils.save_image(g2_output.data, '%s/%s/%s/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 2, epoch),
                          normalize=True)
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            print('Level %d [%d/%d][%d/%d] errD_real: %.4f, errD_fake: %.4f, errG_bce: %.4f errG_mse: %.4f, \
                ...
                d3_real_mean,
                d3_fake_mean))
        torch.save(lapnet3_gen.state_dict(), '%s/%s/%s/d%d_epoch_%d.pth' % (opt.outf, opt.dataset, opt.model, 3, epoch))
        torch.save(lapnet3_disc.state_dict(), '%s/%s/%s/g%d_epoch_%d.pth' % (opt.outf, opt.dataset, opt.model, 3, epoch))
        vutils.save_image(g3_target, '%s/%s/%s/l%d_real_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 3, epoch),
                          normalize=True)
        vutils.save_image(g3_output.data, '%s/%s/%s/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 3, epoch),
                          normalize=True)
        ...
        for idx, (data, _) in enumerate(trainloader, 0):
            ...
            print('Level %d [%d/%d][%d/%d] errD_real: %.4f, errD_fake: %.4f, errG_bce: %.4f errG_mse: %.4f, \
                ...
                d4_real_mean,
                d4_fake_mean))
        torch.save(lapnet4_disc.state_dict(), '%s/%s/%s/d%d_epoch_%d.pth' % (opt.outf, opt.dataset, opt.model, 4, epoch))
        torch.save(lapnet4_gen.state_dict(), '%s/%s/%s/g%d_epoch_%d.pth' % (opt.outf, opt.dataset, opt.model, 4, epoch))
        vutils.save_image(g4_target, '%s/%s/%s/l%d_real_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 4, epoch),
                          normalize=True)
        vutils.save_image(g4_output.data, '%s/%s/%s/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 4, epoch),
                          normalize=True)
