...
def data_loader():
    ...
    if opt.dataset == 'lsun':
        ...
        val_dataset = datasets.ImageFolder(root='/home/user/kaixu/myGitHub/datasets/BSDS500/test/',
                                            transform=transforms.Compose([
                                                transforms.Scale(opt.image_size),
                                                transforms.CenterCrop(opt.image_size),
                                                transforms.ToTensor(),
...
def train(epochs, trainloader, valloader):
    ...
    for epoch in range(epochs):
        ...
        vutils.save_image(g2_output.data,
                          '%s/%s/%s/image/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 2, epoch),
                          normalize=True)
        ...
        vutils.save_image(g3_output.data,
                          '%s/%s/%s/image/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 3, epoch),
                          normalize=True)
        ...
        vutils.save_image(g4_output.data,
                          '%s/%s/%s/image/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 4, epoch),
                          normalize=True)
        ...
        vutils.save_image(g4_target,
                          ...
                          normalize=True)
        vutils.save_image(g4_output.data,
                          '%s/%s/%s/image/l%d_fake_samples_epoch_%02d.png' % (opt.outf, opt.dataset, opt.model, 5, epoch),
                          normalize=True)
