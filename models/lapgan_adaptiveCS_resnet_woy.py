...
class LAPGAN_Generator_level2(nn.Module):
    def __init__(self, channels, leny, ngpu):
        ...
        self.linear1 = nn.Linear(self.base * ((self.fs / 2) ** 2), self.base * (self.fs ** 2))

        self.upsamp2 = nn.ConvTranspose2d(self.base, self.base, kernel_size=4, padding=1, stride=2,
                                          bias=False)  # 64x16x16
        self.bn3 = nn.BatchNorm2d(self.base)

        modules = [ResBlock_wBN(self.base) for _ in range(self.nResBlock)]
        self.resblk1 = nn.Sequential(*modules)
...
class LAPGAN_Generator_level3(nn.Module):
    def __init__(self, channels, leny, ngpu):
        ...
        self.linear1 = nn.Linear(self.base * ((self.fs / 2) ** 2), self.channels * (self.fs ** 2))
