...
class LAPGAN_Generator_level0(nn.Module):
    def __init__(self, channels, ngpu):
        super(LAPGAN_Generator_level0, self).__init__()
        ...
        self.linear1 = nn.Linear(self.channels * self.fs ** 2, self.channels * self.fs **2)
...
class LAPGAN_Generator_level1(nn.Module):
    def __init__(self, channels, ngpu, leny):
        ...
        self.conv1 = nn.Conv2d(self.channels, self.base, kernel_size=3, padding=1, stride=2, bias=False)  # 64x4x4
        self.bn1 = nn.BatchNorm2d(self.base)

        self.linear1 = nn.Linear(self.base * ((self.fs / 2)  ** 2), self.leny)
        self.linear2 = nn.Linear(self.leny * 2, self.channels * (self.fs ** 2))
        self.conv2 = nn.Conv2d(self.channels, self.base, kernel_size=3, padding=1, stride=1, bias=False)
        ...
        self.conv3 = nn.Conv2d(self.base, self.channels, kernel_size=3, padding=1, stride=1, bias=False)  # 3x8x8
        self.tanh = nn.Tanh()

    def forward(self, input, y):
        self.output = self.relu(self.bn1(self.conv1(input)))
...
class LAPGAN_Generator_level2(nn.Module):
    def __init__(self, channels, ngpu, leny):
        ...
        self.linear1 = nn.Linear(self.base * ((self.fs / 2) ** 2), 4 * self.leny)
        self.linear2 = nn.Linear(self.leny * 5, self.channels * (self.fs ** 2))

        self.upsamp2 = nn.ConvTranspose2d(self.channels, self.base, kernel_size=4, padding=1, stride=2, bias=False) # 64x16x16
    ...
    def forward(self, input, y):
        ...
        self.output = self.linear1(self.output)
        self.output = torch.cat((y.view(y.size(0), -1), self.output), 1)
        self.output = self.relu(self.bn(self.linear2(self.output)))
        self.output = self.output.view(-1, self.channels, self.fs, self.fs)
        self.output = self.upsamp2(self.output)
        self.output = self.resblk1(self.output)
...
class LAPGAN_Generator_level3(nn.Module):
    def __init__(self, channels, ngpu, leny):
        ...
        self.linear1 = nn.Linear(self.base * ((self.fs / 2) ** 2), 2 * self.leny)
        self.linear2 = nn.Linear(self.leny * 3, self.channels * (self.fs ** 2))
