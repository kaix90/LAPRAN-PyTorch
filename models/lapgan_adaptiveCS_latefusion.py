...
class LAPGAN_Generator_level1(nn.Module):
    def __init__(self, channels, leny, ngpu):
        ...
        modules = [ResBlock_wBN(self.base, 3) for _ in range(self.nResBlock)]
...
class LAPGAN_Generator_level2(nn.Module):
    def __init__(self, channels, leny, ngpu):
        ...
        self.conv2 = nn.Conv2d(self.base, self.channels, kernel_size=3, padding=1, stride=1, bias=False)
    ...
    def forward(self, input, y):
        ...
        self.output = y.view(y.size(0), -1)
        self.output = self.relu(self.linear1(self.output))
        self.output = self.output.view(-1, self.channels, self.fs, self.fs) # 4x4
        self.output = self.relu(self.bn1(self.conv1(self.output)))

        self.output = self.relu(self.bn2(self.upsamp2(self.output)))
        self.output = self.resblk1(self.output)
...
class LAPGAN_Generator_level3(nn.Module):
    def __init__(self, channels, leny, ngpu):
        ...
        self.upsamp2 = nn.ConvTranspose2d(self.base, self.base, kernel_size=4, padding=1, stride=2,
                                          bias=False)  # 64x16x16
        self.bn2 = nn.BatchNorm2d(self.base)
    ...
    def forward(self, input, y):
        ...
        self.output = y.view(y.size(0), -1)
        self.output = self.relu(self.linear1(self.output))
        self.output = self.output.view(-1, self.channels, self.fs, self.fs)  # 16x16
        self.output = self.relu(self.bn1(self.conv1(self.output)))
        self.output = self.relu(self.bn2(self.upsamp2(self.output)))
        self.output = self.resblk1(self.output)
...
class LAPGAN_Generator_level4(nn.Module):
    def __init__(self, channels, leny, ngpu):
        ...
        self.linear1 = nn.Linear(self.in_features, self.channels * self.fs ** 2)  # 3x32x32
