...
class LAPGAN_Generator_level4(nn.Module):
    def __init__(self, channels, leny, ngpu):
        ...
        self.conv2 = nn.Conv2d(self.base, self.base, kernel_size=3, padding=1, stride=2, bias=False)  # 64x16x16
        self.conv3 = nn.Conv2d(self.base, self.base, kernel_size=3, padding=1, stride=2, bias=False)  # 64x8x8
        self.conv4 = nn.Conv2d(self.base, self.base, kernel_size=3, padding=1, stride=2, bias=False)  # 64x4x4

        self.linear1 = nn.Linear(self.base * (self.fs ** 2), self.leny)
        self.linear2 = nn.Linear(self.leny * 2, self.channels * (self.fs ** 2))

        self.upsamp2 = nn.ConvTranspose2d(self.channels, self.base, kernel_size=4, padding=1, stride=2,
                                          bias=False)  # 64x8x8
        self.upsamp3 = nn.ConvTranspose2d(self.base, self.base, kernel_size=4, padding=1, stride=2,
                                          bias=False)  # 64x16x16
        self.upsamp4 = nn.ConvTranspose2d(self.base, self.base, kernel_size=4, padding=1, stride=2,
                                          bias=False)  # 64x32x32
        self.upsamp5 = nn.ConvTranspose2d(self.base, self.base, kernel_size=4, padding=1, stride=2,
                                          bias=False)  # 64x64x64
    ...
    def forward(self, input, y):
        ...
        self.output = self.output.view(-1, self.channels, self.fs, self.fs)
        self.output = self.relu(self.upsamp2(self.output))
        self.output = self.relu(self.upsamp3(self.output))
        self.output = self.relu(self.upsamp4(self.output))
        self.output = self.relu(self.upsamp5(self.output))
        self.output = self.resblk1(self.output)
...
class LAPGAN_Generator_level4(nn.Module):
    ...
    def forward(self, input, y):
        ...
        self.output = self.relu(self.linear2(self.output))
        self.output = self.output.view(-1, self.channels, self.fs, self.fs)
        self.output = self.relu(self.bn3(self.upsamp2(self.output)))
        self.output = self.resblk1(self.output)
