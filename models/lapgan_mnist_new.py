...
# The implementation of the gen1, input is 3x8x8
class LAPGAN_Generator_level1(nn.Module):
    def __init__(self, channels, ngpu):
        ...
        self.deconv5 = nn.ConvTranspose2d(2 * self.base, self.channels, kernel_size=2, stride=2)  # 3x8x8
        self.tanh = nn.Tanh()
...
# The implementation of the disc1, input is 1x8x8
class LAPGAN_Discriminator_level1(nn.Module):
    def __init__(self, channels, ngpu):
        ...
        self.main = nn.Sequential(
            nn.Conv2d(self.channels, self.base, kernel_size=2, stride=2, bias=False),  # 32x4x4
            nn.BatchNorm2d(self.base),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(self.base, 2 * self.base, kernel_size=2, stride=2, bias=False),  # 64x2x2
            nn.BatchNorm2d(2 * self.base),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(2 * self.base, 1, kernel_size=2, stride=1, bias=False),
            nn.Sigmoid()
        )
...
class LAPGAN_Generator_level2(nn.Module):
    def __init__(self, channels, ngpu, leny):
        ...
        self.bn3 = nn.BatchNorm2d(4 * self.base)
        self.conv4 = nn.Conv2d(4 * self.base, 8 * self.base, kernel_size=2, stride=2)  # 512x2x2
        self.linear = nn.Linear(8 * self.base * 2 * 2 + leny, 8 * self.base * 2 * 2)

        self.deconv5 = nn.ConvTranspose2d(8 * self.base, 4 * self.base, kernel_size=2, stride=2)  # 256x4x4
        self.bn5 = nn.BatchNorm2d(4 * self.base)
        self.deconv6 = nn.ConvTranspose2d(4 * self.base, 2 * self.base, kernel_size=2, stride=2)  # 128x8x8
        self.bn6 = nn.BatchNorm2d(2 * self.base)
        self.deconv7 = nn.ConvTranspose2d(2 * self.base, self.channels, kernel_size=2, stride=2)  # 3x16x16
        self.tanh = nn.Tanh()
    ...
    def forward(self, input, y):
        ...
        output = torch.cat((output.view(-1, 8 * self.base * 2 * 2), y.view(-1, self.channels * 8 * 8)), 1)
        output = self.linear(output)
        output = output.view(-1, 8 * self.base, 2, 2)
...
class LAPGAN_Generator_level4(nn.Module):
    def __init__(self, channels, ngpu, leny):
        ...
        self.linear = nn.Linear(16 * self.base * 4 * 4 + leny, 8 * self.base * 4 * 4)
